...
- PromptHub 自有的 AI HTTP 请求统一使用 `packages/shared/constants/ai.ts` 定义的 300 秒有界 deadline，覆盖模型发现、连接测试、聊天、翻译、图像生成以及响应正文或流读取；不得为单个 AI 操作维护更短的本地常量。该约束不适用于 Agent provider 探测、同步、下载或其他非 AI transport 网络请求。
